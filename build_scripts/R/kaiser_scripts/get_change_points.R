...
condition_name <- stringr::str_to_title(str_remove(condition, "_kaiser"))
...
# load SSD codes
ssd_codes <- bind_rows(codeBuildr::load_ssd_codes(delay_params$ssd_name) %>%
                         filter(type == "icd9") %>%
                         select(dx=code) %>%
                         mutate(icd_ver=9),
                       codeBuildr::load_ssd_codes(delay_params$ssd_name) %>%
                         ...
                         select(dx=code) %>%
                         mutate(icd_ver=10)) %>% 
  mutate(icd_ver = str_pad(icd_ver, width = 2, side = "left", pad = "0"))
...
ssd_set <- load_ssd_codes(delay_params$ssd_name) %>%
...
rmarkdown::render(input = "/Shared/Statepi_Diagnosis/atlan/github/delay_diagnosis/build_scripts/R/kaiser_scripts/opp_window_report.Rmd",
                  params = list(cond = condition,
                                condition_name = condition_name),
                  output_dir = out_path)

rmarkdown::render(input = "/Shared/Statepi_Diagnosis/atlan/github/delay_diagnosis/build_scripts/R/kaiser_scripts/test_opp_window_report.Rmd",
                  params = list(cond = condition,
                                condition_name = condition_name),
                  output_dir = out_path)
