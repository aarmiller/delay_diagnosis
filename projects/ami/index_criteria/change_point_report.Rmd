...
# load delay_parms
load("~/Data/Statepi_Diagnosis/projects/ami/sex_ssd_counts.RData")
...
# count ssd visits men
count_data_men <- sex_ssd_counts %>% 
  filter(sex == 1) %>% 
...
# count ssd visits women
count_data_women <- sex_ssd_counts %>% 
  filter(sex == 2) %>% 
  filter(days_since_index<0) %>% 
  mutate(period = -days_since_index) %>% 
  mutate(period = period - 1) %>% 
...
```
This document provides a summary of the change-point analysis for AMI, broken down
by men and women

# Male visits
The following section summarizes change-point analysis for trends in the number 
of SSD-related visits prior to the index diagnosis for men

```{r, include=FALSE}

# piecewise model results
ssd_res1_men <- tibble(model = c("lm","quad","cubic","exp")) %>% 
...
# cusum model results
ssd_res2_men <- tibble(model = c("lm","quad","cubic","exp")) %>% 
  mutate(label = c("Linear", "Quadratic", "Cubic", "Exponential")) %>% 
  mutate(periodicity = map(model,~c(T,F))) %>% 
  unnest(periodicity) %>% 
  mutate(label = ifelse(periodicity==TRUE,paste0("CUSUM ",label," w/ periodicity"),label)) %>% 
  mutate(cp_res = map2(model,periodicity,
                       ~fit_cumsum_mods(count_data_men,model = .x, periodicity = .y))) %>% 
  mutate(cp = map_dbl(cp_res,~.$cp)) %>% 
  mutate(pred = map(cp_res,~.$pred)) %>% 
  select(model,label,periodicity,cp,pred) %>% 
  mutate(pred_bound_cp = map_int(pred,find_pred_bound_cp)) %>% 
...
ssd_res_out_men <- bind_rows(mutate(ssd_res1_men,
...
```{r}
opt_mods <- get_rankings(ssd_res_out_men) %>% 
...
```{r}
tmp <- bind_rows(mutate(ssd_res1_men,
                        tmp = ifelse(periodicity," w/ periodicity",""),
                        label = paste0("Piecewise ", model, tmp)),
                 mutate(ssd_res2_men,
                        tmp = ifelse(periodicity," w/ periodicity",""),
...
# Female Visits
The following section summarizes change-point analysis for trends in the number 
of SSD visits for women prior to the index diagnosis


```{r, include=FALSE}

# piecewise model results
ssd_res1_women <- tibble(model = c("lm","quad","cubic","exp")) %>% 
  mutate(label = c("Linear", "Quadratic", "Cubic", "Exponential")) %>% 
  mutate(periodicity = map(model,~c(T,F))) %>% 
  unnest(periodicity) %>% 
  mutate(label = ifelse(periodicity==TRUE,paste0(label," w/ periodicity"),label)) %>% 
  mutate(cp_res = map2(model,periodicity,
                       ~find_cp(count_data_women,
                                cp_range = 3:300,
                                model = .x,
                                periodicity = .y))) %>% 
  mutate(cp = map_dbl(cp_res,~.$cp)) %>% 
...
# cusum model results
ssd_res2_women <- tibble(model = c("lm","quad","cubic","exp")) %>% 
  mutate(label = c("Linear", "Quadratic", "Cubic", "Exponential")) %>% 
  mutate(periodicity = map(model,~c(T,F))) %>% 
  unnest(periodicity) %>% 
  mutate(label = ifelse(periodicity==TRUE,paste0("CUSUM ",label," w/ periodicity"),label)) %>% 
  mutate(cp_res = map2(model,periodicity,
                       ~fit_cumsum_mods(count_data_women,model = .x, periodicity = .y))) %>% 
  mutate(cp = map_dbl(cp_res,~.$cp)) %>% 
  mutate(pred = map(cp_res,~.$pred)) %>% 
  select(model,label,periodicity,cp,pred) %>% 
  mutate(pred_bound_cp = map_int(pred,find_pred_bound_cp)) %>% 
...
# aggregate results output
ssd_res_out_women <- bind_rows(mutate(ssd_res1_women,
                                 tmp = ifelse(periodicity," w/ periodicity",""),
                                 label = paste0("Piecewise ", model, tmp)),
                          mutate(ssd_res2_women,
                                 tmp = ifelse(periodicity," w/ periodicity",""),
                                 label = paste0("CUSUM ", model, tmp))) %>% 
  select(label,cp,pred_bound_cp,pred) %>% 
  mutate(pred = map(pred,~mutate(., n_miss = ifelse(n>pred1,n-pred1,0)))) %>% 
...
```{r}
opt_mods <- get_rankings(ssd_res_out_women) %>% 
  filter(avg_rank == min(avg_rank))
...
```{r}
tmp <- bind_rows(mutate(ssd_res1_women,
                        tmp = ifelse(periodicity," w/ periodicity",""),
                        label = paste0("Piecewise ", model, tmp)),
                 mutate(ssd_res2_women,
                        tmp = ifelse(periodicity," w/ periodicity",""),
...
```{r}
get_rankings(ssd_res_out_women) %>% 
  select(label,`Change Point`=cp,`Pred. Bound CP` = pb_cp,`Average Rank`=avg_rank,consistency_rank:mse14_rank) %>% 
  kableExtra::kable() %>% 
  kableExtra::kable_styling(full_width = F)
```
