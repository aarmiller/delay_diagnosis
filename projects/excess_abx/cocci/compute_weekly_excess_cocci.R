...
cocci_cw <- index_dates %>% 
  inner_join(crosswalk)

save(cocci_cw, file = "/Volumes/AML/tmp_transfer/excess_abx/cocci_index_cases.RData")

## Pull Medicaid RX enroll On Argon --------------------------------------------

load("/Shared/AML/tmp_transfer/excess_abx/cocci_index_cases.RData")
...
med_rx_enroll <- tmp_enroll %>% 
  unnest(data) %>% 
  inner_join(select(cocci_cw,patient_id,enrolid)) %>% 
  select(-year)

save(med_rx_enroll, file = "/Shared/AML/tmp_transfer/excess_abx/cocci_med_rx_enroll.RData")
