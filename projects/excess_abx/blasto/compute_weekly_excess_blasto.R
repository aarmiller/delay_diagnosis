...
abx_include <- abx_list %>% 
  ...
  # filter(name!="amoxicillin") %>%  # remove due to negative in all models
  filter(!(name %in% c("cefaclor","cefditoren","cefixime","cefpodoxime",
                       "cefprozil","cefadroxil","clarithromycin","amoxicillin",
                       "cefuroxime","clarithromycin","moxifloxacin"))) %>% 
  select(name)
...
blasto_cw <- index_dates %>% 
  inner_join(crosswalk)

save(blasto_cw, file = "/Volumes/AML/tmp_transfer/excess_abx/blasto_index_cases.RData")

## Pull Medicaid RX enroll On Argon --------------------------------------------

load("/Shared/AML/tmp_transfer/excess_abx/blasto_index_cases.RData")
...
med_rx_enroll <- tmp_enroll %>% 
  unnest(data) %>% 
  inner_join(select(blasto_cw,patient_id,enrolid)) %>% 
  select(-year)

save(med_rx_enroll, file = "/Shared/AML/tmp_transfer/excess_abx/blasto_med_rx_enroll.RData")
...
save(index_dates,
     ...
     agg_boot_res,
     file = "~/Data/projects/excess_abx/blasto/data/report_data.RData")
...
save(final_abx_counts,
     ...
     tmp_excess_quad,
     tmp_excess_cubic,
     file = "~/Data/projects/excess_abx/blasto/data/simulation_results.RData")
