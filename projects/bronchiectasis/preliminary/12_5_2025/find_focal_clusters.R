...
out_path <- paste0(base_path,"/fatigue_10_included/")
...
hemoptysis_10 <- find_focal_clusters(cluster_data = clust_res_10,
                                     focal_code = "R042",
                                     dx_ver = 10L)


hemoptysis_9 <- find_focal_clusters(cluster_data = clust_res_9,
                                    focal_code = "7863",
                                    dx_ver = 9L)
...
fatigue_10_1 <- find_focal_clusters(cluster_data = clust_res_10,
                                    focal_code = "R5383",
                                    dx_ver = 10L)

fatigue_10_2 <- find_focal_clusters(cluster_data = clust_res_10,
                                    focal_code = "R5381",
                                    dx_ver = 10L)


fatigue_9 <- find_focal_clusters(cluster_data = clust_res_9,
                               focal_code = "78079",
                               dx_ver = 9L)
...
focal_cluster_res_10 <- bind_rows(mutate(cough_10$condition_counts,term = "cough"),
                                  ...
                                  mutate(fatigue_10_1$condition_counts,term = "fatigue1"),
                                  mutate(fatigue_10_2$condition_counts,term = "fatigue2"))
...
focal_cluster_res_10 %>% 
  ...
  write_csv(paste0(out_path,"cluster_res_icd_10.csv"))
...
focal_cluster_res_9 <- bind_rows(mutate(cough_9$condition_counts,term = "cough"),
                                  ...
                                  mutate(hemoptysis_9$condition_counts,term = "hemoptysis"),
                                  mutate(weight_loss_9$condition_counts,term = "weight_loss"),
                                  mutate(fatigue_9$condition_counts,term = "fatigue"))
...
focal_cluster_res_9 %>% 
  ...
  arrange(desc(total),rank) %>% 
  ungroup() %>% 
  mutate(cluster_frac = round(100*total/(nrow(distinct(focal_cluster_res_9,term))*28),2))  %>% 
  select(code,desc,`Overall Rank`=rank,`Total Clusters`=total,`Cluster Fraction`=cluster_frac) %>% 
  write_csv(paste0(out_path,"cluster_res_icd_9.csv"))
...
pdf(paste0(out_path,"cluster_plots_icd9.pdf"),onefile = TRUE)
...
pdf(paste0(out_path,"cluster_plots_icd10.pdf"),onefile = TRUE)
...
excluded_codes9 %>% 
  inner_join(filter(codeBuildr::all_icd_labels, dx_ver==9) %>% 
               select(code = dx, desc)) %>% 
  write_csv(paste0(out_path,"cluster_res_excluded_icd9.csv"))
...
excluded_codes10 %>% 
  left_join(filter(codeBuildr::all_icd_labels, dx_ver==10) %>% 
               select(code = dx, desc)) %>%  
  write_csv(paste0(out_path,"cluster_res_excluded_icd10.csv"))


pdf(paste0(out_path,"excluded_plots_icd9.pdf"),onefile = TRUE)
...
pdf(paste0(out_path,"cluster_excluded_plots_icd10.pdf"),onefile = TRUE)
