...
load("~/Data/params/delay_any_params.RData")
...
clust_res_9_pre <- find_pred_clusters(filter(dx9_fits,days_since_index<0), k_range = 3:30)
clust_res_9_post <- find_pred_clusters(filter(dx9_fits,days_since_index>0), k_range = 3:30)


### ICD-10 Clusters -------------------------------------------------------------

clust_res_10 <- find_pred_clusters(dx10_fits, k_range = 3:30)
clust_res_10_pre <- find_pred_clusters(filter(dx10_fits,days_since_index<0), k_range = 3:30)
clust_res_10_post <- find_pred_clusters(filter(dx10_fits,days_since_index>0), k_range = 3:30)

if (!dir.exists(paste0(cluster_out_path,str_replace_all(Sys.Date(),"-","_")))){
...
tmp_dx9 <- dx9_counts_nested %>% 
  ...
  ungroup() %>% 
  mutate(label = paste0("ICD9 - ",code,": ",str_sub(description,1,40)))

pdf(paste0(plot_path,"top_2000_dx9.pdf"),onefile = TRUE)
for (i in 1:ceiling(2000/6)){
  tmp_conds <- icd9_ranks %>%
    slice((i*6-5):(i*6)) %>% 
    select(code)
...
tmp_dx10 <- dx10_counts_nested %>% 
  ...
  ungroup() %>% 
  mutate(label = paste0("ICD10 - ",code,": ",str_sub(description,1,40)))

pdf(paste0(plot_path,"top_2000_dx10.pdf"),onefile = TRUE)
for (i in 1:ceiling(2000/6)){
