...
### Note: The year 2001 were excluded from the analyses below as no index visits occured during that year. This has to do with the 365 day enrollment requirement.
...
between index and their earliest missed opportunity. If no delay occurred the 
duration was 0 days.**This analysis utilizes a linear regression model with duration 
...
of delays. In each model patients were assigned an integer value based on days 
between index and their earliest missed opportunity. If no delay occurred the 
duration was 0 days. **This analysis utilizes a Log-normal accelerated failure time model.
...
A small $\ \epsilon$ (i.e., 0.01) was added to each delay duration to allow for 
...
of delays. In each model patients were assigned an integer value based on days 
between index and their earliest missed opportunity. If no delay occurred the 
duration was 0 days. **This analysis utilizes a Weibull accelerated failure time model.** 
In order to fit the model, a small $\ \epsilon$ (i.e., 0.01) was added to each delay duration.
...
by a multiplicative factor of 1.6. Note: this model and the log-normal ATF model assume that those not selected to have a missed opportunity in a given trial are assumed
to have their earliest SSD occur an $\ \epsilon$ amount time before their index.**
