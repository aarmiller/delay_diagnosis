...
order <- miss_delay_dur_LMM_RE %>%
  arrange(desc(intercept_est))

temp_data <- miss_delay_dur_LMM_RE %>%
  mutate(state_abb2 = factor(state_abb, levels = order$state_abb, ordered = T))

temp_data %>%
ggplot(aes(y = intercept_est,x = state_abb2))+
  geom_point()+
  geom_errorbar(aes(ymin = low,ymax = high), position = position_dodge(0.9), width = 0.2) +
  ylab("Predicted Random Intercept")+
  xlab("State")+
  ggtitle("LMM for Duration of Delay: Predicted State-level Random Intercept (95% percentile-based CI)")+
...
ggplot(aes(y = intercept_est,x = state_abb2))+
  geom_point()+
  geom_errorbar(aes(ymin = low_90,ymax = high_90), position = position_dodge(0.9), width = 0.2) +
  ylab("Predicted Random Intercept")+
  xlab("State")+
  ggtitle("LMM for Duration of Delay: Predicted State-level Random Intercept (90% percentile-based CI)")+
  geom_hline(yintercept=0, linetype="dashed", color = "red")+
  theme_bw()+
  ylim(-12, 15)
