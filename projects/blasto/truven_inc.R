...
## Load all blasto dx visits
load("/Shared/AML/truven_extracts/dx/blasto/blasto_dx_visits.RData")
load("/Shared/AML/truven_extracts/dx/blasto/blasto_enrolids.RData")
...
all_blasto_dx_visits <- all_outpatient_visits %>% distinct(enrolid, svcdate) %>% select(enrolid, admdate = svcdate) %>% 
...
# 6125 distinct enrollees all_blasto_dx_visits %>% distinct(enrolid), matches extract reports

# add year
all_blasto_dx_visits <- all_blasto_dx_visits %>% 
  mutate(year = as.character(year(as_date(admdate)))) 

# load crosswalk
load("/Shared/AML/truven_extracts/small_dbs/blasto/blasto_enrolid_crosswalk.RData")
...
enroll_collapsed_temp2 <- enroll_collapsed_temp %>% 
  ...
  inner_join(select(all_blasto_dx_visits,patient_id,admdate, year)) %>% 
  filter(admdate<=dtend & admdate>=dtstart) %>% 
  distinct() 
# only 5606 out of 6074 have location information, which matches exactly to non-medicaid enrolids %>% count(medicaid) vs enroll_collapsed_temp2 %>% distinct(patient_id)
 
all_blasto_dx_visits <- all_blasto_dx_visits %>% 
  left_join(enroll_collapsed_temp2 %>% select(patient_id, egeoloc , admdate, year))

count_blasto_by_egeoloc_year <- all_blasto_dx_visits %>% 
  distinct() %>% 
  count(year, egeoloc) %>% 
  inner_join(egeoloc_labels %>% select(egeoloc, location, state_name, state_abb)) 
...
truven_blasto_inc <- truven_enrollement_by_egeoloc_year %>% 
  rename(n_total = n) %>% 
  mutate(year = paste0("20", year)) %>% 
  left_join(., count_blasto_by_egeoloc_year %>% rename(n_cases = n) %>% 
...
ave_truven_blasto_inc_state <- truven_blasto_inc %>% 
...
# save 
save(truven_blasto_inc, ave_truven_blasto_inc_state, file = paste0(out_path,"truven_inc_by_egeoloc_year.RData")) 
...
states_w_inc <- usmap::statepop %>% select(-pop_2015) %>%
  inner_join(ave_truven_blasto_inc_state %>% rename(abbr = state_abb))

plot_usmap(data = states_w_inc, values = "ave_inc_per_100000")+
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", name =  "",
                       label = scales::comma) +
  labs(title = "Truven Blastomycosis Incidence per 100,000", subtitle = "Average annual incidence over 2001-2022") +
...
plot_usmap(data = top5, values = "top5")+
  scale_fill_gradient2(low = "white",high = "green", name =  "",
                       label = scales::comma) +
  labs(title = "Top 5 - Truven Blastomycosis Incidence per 100,000", subtitle = "Average annual incidence over 2001-2022") +
...
plot_usmap(data = top10, values = "top10")+
  scale_fill_gradient2(low = "white",high = "green", name =  "",
                       label = scales::comma) +
  labs(title = "Top 10 - Truven Blastomycosis Incidence per 100,000", subtitle = "Average annual incidence over 2001-2022") +
  theme(legend.position = "none")
