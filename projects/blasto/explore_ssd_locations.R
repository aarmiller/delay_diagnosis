...
ssd_MN_WI_fun <- function(trial_val){
  ...
  tibble(WI_MN = c(0,1,NA)) %>% 
    left_join(temp2) %>% 
    mutate_at(vars(n, percent),~replace_na(.,0L))
}

ssd_MN_WI <- parallel::mclapply(1:max(full_reg_data$trial),
                                     function(x){ssd_MN_WI_fun(x)}, 
                                     mc.cores = num_cores)


ssd_MN_WI <- bind_rows(ssd_MN_WI) %>% 
  group_by(WI_MN) %>% 
  summarise(n_mean = mean(n, na.rm = T),
            n_low = quantile(n, probs = c(0.025), na.rm = T),
            n_high = quantile(n, probs = c(0.975), na.rm = T),
...
write_csv(ssd_MN_WI, file = paste0(out_path,"ssd_counts_MN_WI.csv"))  
...
ssd_WI_MN_fun <- function(trial_val){
...
ssd_WI_MN <- parallel::mclapply(1:max(full_reg_data$trial),
                                  function(x){ssd_WI_MN_fun(x)}, 
                                  mc.cores = num_cores)


ssd_WI_MN <- bind_rows(ssd_WI_MN) %>% 
  group_by(WI_MN) %>% 
  summarise(ssd_time_max = mean(duration_max, na.rm = T),
            ssd_time_max_low = quantile(duration_max, probs = c(0.025), na.rm = T),
            ssd_time_max_high = quantile(duration_max, probs = c(0.975), na.rm = T),
...
write_csv(ssd_WI_MN, file = paste0(out_path,"ssd_duration_WI_MN.csv"))  
...
# WI_MN
ssd_count_state <- read_csv(file = paste0(out_path,"ssd_counts_MN_WI.csv"))  
index_count_state <- read_csv(file = paste0(out_path,"index_WI_MN_count.csv"))  
...
# wont sum to 100% by group as excluding NA
data %>% filter(!is.na(WI_MN)) %>% 
  mutate(new_lab=ifelse(WI_MN == 1, "yes", "no")) %>% 
  ggplot(aes(x = new_lab, y = percent, fill = group))+
  geom_bar(stat="identity", position=position_dodge())+
  ylab("Percent")+
  xlab("MN or WI")+
  ggtitle("Percent of visits by in MN or WI vs. all others")
...
ssd_time <- read_csv(file = paste0(out_path,"ssd_duration_WI_MN.csv"))  

data <- ssd_time %>%
  filter(!is.na(WI_MN)) %>% 
  select(WI_MN, ssd_time_min, ssd_time_q1, ssd_time_median, ssd_time_q3, ssd_time_max) %>% 
  pivot_longer(cols = ssd_time_min:ssd_time_max, values_to = "y", names_to = "group") %>% 
  mutate(group = str_remove_all(group, "ssd_time_")) %>% 
  mutate(new_lab=ifelse(WI_MN == 1, "yes", "no")) 
...
data %>% 
  ...
  ylab("SSD time before index")+
  xlab("MN or WI")+
  ggtitle("SSD time before index by MN or WI vs. all others")+ 
  guides(fill="none")
