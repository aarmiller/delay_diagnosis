...
# reg_demo %>% count(vaccination_prior_cp)%>% 
#   mutate(percent = n/sum(n)*100)
# # A tibble: 2 x 3
# vaccination_prior_cp     n percent
# <dbl> <int>   <dbl>
#   1                    0  4400   94.6 
# 2                    1   251    5.40

# reg_demo %>% filter(vaccination_prior_cp==1) %>% count(age_cat) %>% 
#   mutate(percent = n/sum(n)*100)
# age_cat      n percent
# <fct>    <int>   <dbl>
#   1 (-1,1]      11   4.38 
...
#### Delay Patient All - firth ---------------------------------------------------------
library(logistf)

get_delay_pat_res_firth <- function(trial_val){
  ...
  reg_data <- tmp1 %>% 
    ...
    return(tibble(term = names(coef(fit)),
                  estimate = coef(fit)))
...
# miss_delay_pat_res_firth <- parallel::mclapply(1:max(full_reg_data$trial),
#                                          function(x){get_delay_pat_res_firth(x)},
#                                          mc.cores = num_cores)
...
miss_delay_pat_res_firth <- parallel::mclapply(1:max(full_reg_data$trial),
                                               function(x){
                                                 ...
                                                 results <- tryCatch({
                                                   get_delay_pat_res_firth(x)
                                                   
                                                 }, error = function(e) {
                                                   
                                                   message("Model failed: ", x, e$message)
                                                   return(NULL)
...
# miss_delay_pat_res_firth <- lapply(1:max(full_reg_data$trial),
#                                    function(x){
#                                      
#                                      results <- tryCatch({
#                                        
...
missing <- sapply(miss_delay_pat_res_firth, is.null)
sum(missing)


miss_delay_pat_res_firth <- bind_rows(miss_delay_pat_res_firth) %>% 
  group_by(term) %>% 
  summarise(est = median(exp(estimate), na.rm = T),
            low = quantile(exp(estimate),probs = c(0.025), na.rm = T),
            high = quantile(exp(estimate),probs = c(0.975), na.rm = T),
...
ssd_miss_risk_models <- list(miss_opp_res = miss_opp_res,
                             ...
                             miss_delay_pat_res_firth = miss_delay_pat_res_firth
                             )
...
rmarkdown::render(input = paste0("/Shared/Statepi_Diagnosis/atlan/github/delay_diagnosis/projects/measles/measles_validated/risk_model_report.Rmd"),
