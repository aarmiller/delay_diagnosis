...
better_labels <- tribble(~term,~label,
                         ...
                         "fever_100", "Temperature >=100F recorded during visit",
                         "fever_100_or_dxTRUE", "Temperature >=100F or fever dx recorded during visit")
...
The following risk model pertains to all observations (CCAE, MDCR and Medicaid) using a fever indicator constructed as either a temperature >=100F or fever dx recorded during the visit (Note, missing values can be generated particularly if the temperature reading is missing and there is no fever dx recorded during the visit. Missing observations are removed here)

```{r, echo = FALSE}

ssd_miss_risk_models$miss_opp_res_fever_100_or_dx %>% 
...
The following risk model pertains to all observations (CCAE, MDCR and Medicaid) using a fever indicator constructed as a temperature >=100F during the visit (Note, observations with a missing temperature reading are removed here)

```{r, echo = FALSE}

ssd_miss_risk_models$miss_opp_res_fever_100 %>% 
  mutate(`Odds Ratio` = paste0(formatC(est, digits = 3, format = "g", flag= "#"),
                               " (",
                               formatC(low, digits = 3, format = "g", flag= "#"),
                               "-",
...
The following risk model pertains to all observations (CCAE, MDCR and Medicaid) using a fever indicator constructed as a temperature >=100F during the visit, where missing temperature readings are imputed as no fever.

```{r, echo = FALSE}

ssd_miss_risk_models$miss_opp_res_fever_100_na_0 %>% 
  mutate(`Odds Ratio` = paste0(formatC(est, digits = 3, format = "g", flag= "#"),
                               " (",
                               formatC(low, digits = 3, format = "g", flag= "#"),
                               "-",
