...
# 
# temp1 <- expand_data_w_BC %>% 
#   anti_join(BC_data %>% inner_join(temp_data %>% 
#                                      group_by(STUDYID) %>% 
#                                      filter(measurement_date == min(measurement_date)) %>% 
#                                      ungroup()) %>% 
#               filter(procdate < measurement_date) %>% 
#               distinct(STUDYID)) %>%
#   filter(window >= measurement_date & window <= measurement_date + j) %>% 
#   group_by(STUDYID, measurement_date, temp) %>%
#   summarise(BC_window = as.numeric(any(!is.na(BC))), .groups = "drop") %>%
#   mutate(fever = as.numeric(temp >= 101))
# 
# cat("Window = ", j, "\n\n")
# cont_table <- table("Fever" = factor(temp1$fever, levels = c(1,0)), "Blood Culture" = factor(temp1$BC_window, levels = c(1,0)))
...
expand_data <- first_non_missing_temp %>% 
  left_join(BC_data %>% 
              group_by(STUDYID) %>% 
              summarise(procdate = min(procdate), # find when their first BC date occured
                        .groups = "drop") %>% 
