...
abx_codes <- read_csv("/Shared/Statepi_Diagnosis/projects/dengue/antibiotics.csv")
...
get_miss_res_no_setting <- function(trial_val){
  ...
  fit <- logistf(miss~age_cat + female + source + weekend + abx + opioid + ID_consult + year + month, 
                 family = "binomial", data=reg_data,
...
write_csv(table4, paste0("/Shared/Statepi_Diagnosis/atlan/github/delay_diagnosis/publications/", "dengue", "/tables/table4_validated.csv"))
