...
main_fun <- function(proj_name, cp){
  ...
  table1_fun <- function(reg_demo){
    ...
    age <- bind_rows(tibble("Characteristic" = "Age at diagnosis",
                     ...
                     tibble("Characteristic" = " Age Median (IQR)",
    ...
    enroll_time <- bind_rows(tibble("Characteristic" = "Enrollment time prior to index (years)",
                                    "Total Patients (% of patients)" = ""),
                             tibble("Characteristic" = " enroll_time Mean (SD)",
                             ...
                             tibble("Characteristic" = " enroll_time Median (IQR)",
  ...
  miss_bins_ssd_new <- miss_bins_ssd %>% 
    ...
    mutate(N_pat =  n_new,
           ...
           N_pat_upper = n_upper) %>% 
    rowwise() %>% 
    mutate(estimate = paste0(format(N_pat, big.mark = ","), "\n(", format(round(N_pat/n_total *100, 1), nsmall = 1), "%)"),
  ...
  for(i in final_delay_params[[proj_name]]$cp){
    ...
    percent_patient_delayed <- miss_bins_ssd %>% 
      mutate(miss_bin = paste0(">=",miss_bin)) %>% 
      select(miss_bin, n) %>% 
      mutate(N_pat =  n_new,
             N_pat_low = n_lower,
      ...
      mutate(nice = paste0(format(round(N_pat/n_total *100, 1), nsmall = 1),
                           ...
                           format(round(N_pat_upper/n_total *100, 1), nsmall = 1),")")) %>% 
      filter(miss_bin == ">=1") %>% 
      mutate(miss_bin = "percent_patient_delayed") %>% 
      select(metric = miss_bin, nice)
    
    names(percent_patient_delayed)[2] <- paste0("delay_window_1_", i-1)
    ...
    temp_table <- bind_rows(percent_patient_delayed,
                            miss_bins_ssds_mean_median,
                            dur_bins_ssd_mean_median)
    if(nrow(table4) == 0){
      table4 <- temp_table
    } else {
      table4 <- inner_join(table4, temp_table)
    }
