...
for(i in final_delay_params[[proj_name]]$cp){
  ...
  if(nrow(appendix_table1) == 0){
    appendix_table1 <- temp_table
  } else {
    appendix_table1 <- inner_join(appendix_table1, temp_table)
  }
  
}

write_csv(appendix_table1, paste0("/Shared/Statepi_Diagnosis/atlan/github/delay_diagnosis/publications/", proj_name, "/tables/appendix_table2.csv"))
